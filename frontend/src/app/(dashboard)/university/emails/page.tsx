...
import { Bell, CheckCheck, Circle, Mail } from 'lucide-react';
...
const TYPE_CONFIG: Record<string, { badge: string; dot: string }> = {
  ...
  sms:       { badge: 'bg-purple-100 text-purple-700', dot: 'bg-purple-500' },
...
export default function UniversityEmailsPage() {
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Messages from the riftApply admissions team.</p>
        </div>
        {unread > 0 && (
          <button
            onClick={() => {
      ...
      <div className="flex flex-col lg:flex-row gap-5">
        <div className="flex-1 bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          {loading ? <PageLoader /> : msgs.length === 0 ? (
            <EmptyState
              ...
              description="You'll receive notifications here from the admissions team."
            />
          ) : (
        ...
        {selected ? (
          <div className="w-full lg:w-[380px] lg:flex-shrink-0 bg-white rounded-xl border border-gray-200 p-5">
            <span className={`text-[11px] font-semibold px-2 py-0.5 rounded ${TYPE_CONFIG[selected.type]?.badge ?? 'bg-gray-100 text-gray-600'}`}>
              {selected.type}
            </span>
            <h2 className="text-sm font-bold text-gray-900 mt-3 leading-snug">{selected.subject}</h2>
          ...
          <div className="hidden lg:flex w-[380px] flex-shrink-0 bg-white rounded-xl border border-gray-200 items-center justify-center">
